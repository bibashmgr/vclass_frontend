...
const User = () => {
    ...
    const changeUserStatus = () => {
        if (selectedUser) {
            apiHandler('patch', `faculties/status/${selectedUser._id}`).then(
                (res) => {
                    if (res.success) {
                    ...
                    users.map((user: userSchema, userIndex) => {
                        return (
                            <tr key={user._id} className="bg-lightColor dark:bg-gray-800">
                                ...
                                <ActionTd hasView hasEdit hasArchive isHidden={user.isHidden} handleView={() => navigate(`/admin/user/view/${user._id}`)} handleEdit={() => navigate(`/admin/user/edit/${user._id}`)} handleArchive={() => {
